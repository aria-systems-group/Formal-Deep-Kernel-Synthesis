...
class LiRPAConvNet:
    ...
    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None, stop_criterion_func=stop_criterion_sum(0), bounding_method=None):
        ...
        self.net.set_bound_opts({'verbosity': 0})
        self.net.set_bound_opts({'optimize_bound_args': {
        ...
        if bounding_method == "alpha-crown":
            ...
            # REED MOD
            lb, ub, aux_reference_bounds = self.net.init_slope(
                (self.x,), share_slopes=share_slopes, c=None, bound_upper=True)

            if len(self.layers) == 5:
                self.needed_A_dict = {'/15': ['/input']}
            elif len(self.layers) == 4:
                self.needed_A_dict = {'/11': ['/input']}
            else:
                self.needed_A_dict = {'/7': ['/input']}

            lb, ub, A_dict = self.net.compute_bounds(x=(x,), C=None, method='CROWN-Optimized',
            ...
            if len(self.layers) == 5:
                outputs = A_dict['/15']['/input']
            elif len(self.layers) == 4:
                outputs = A_dict['/11']['/input']
            else:
                outputs = A_dict['/7']['/input']
            ...
            save_path = 'bounds_{}'.format(str(new_temp))
            np.save(save_path, np.array([lb.detach().numpy(), ub.detach().numpy()]))

            save_path = 'transform_mats_{}'.format(str(new_temp))
            np.save(save_path, np.array([lA.detach().numpy(), uA.detach().numpy()]))

            save_path = 'bias_mats_{}'.format(str(new_temp))
            np.save(save_path, np.array([lbias.detach().numpy(), ubias.detach().numpy()]))
