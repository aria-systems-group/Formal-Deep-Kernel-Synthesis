...
keys = list(X)
if reuse_regions and os.path.exists(global_exp_dir + "/transition_probs.pkl") and not run_tests:
...
# sub_idx = 1
# dim = 0
# region_idx = 13525  # 13528
# # region_idx = len(extents) - 1
# get_one_region(unknown_dyn_gp[mode], region_idx, dim, sub_idx, mode, extents, region_info[mode], out_dim,
#                crown_dir, EXPERIMENT_DIR)
# grow_one_region(unknown_dyn_gp[mode], region_idx, dim, sub_idx, mode, extents, region_info[mode], out_dim,
#                crown_dir, EXPERIMENT_DIR)
# mod_one_region(unknown_dyn_gp[mode], unknown_modes_list[mode], region_idx, dim, sub_idx, mode,
#                extents, region_info[mode], out_dim, crown_dir, EXPERIMENT_DIR)
...
if run_tests:
    ...
    for mode in modes:
        if use_local_gp:
            region_gps = unknown_dyn_gp[mode]
            for region_idx in range(len(region_gps)):
                gp_stuff = region_gps[region_idx]
                num_true = 0
                total_num = 30000
                max_dist_from_mean = 0
                min_dist_from_mean = 9e9
                max_2_sig = 0
                min_2_sig = 9e9
                sig_2_dict = {key: 0 for key in keys}
                sig_2_dict_min = {key: 9e9 for key in keys}

                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                test_domain = region_info[mode][region_idx][2]

                tic = time.perf_counter()
                dim_first_pass = np.ones([d, 1])
                for nums in range(total_num):
                    x_testing = [np.random.uniform(test_domain[dim][0], test_domain[dim][1], 1) for dim, k in enumerate(keys)]
                    x_torch = torch.tensor(np.reshape(x_testing, [1, d]), dtype=torch.float32).to(device)
                    true_dyn = unknown_modes_list[mode]
                    true_y = true_dyn(x_testing)
                    ...
                    for dim, key in enumerate(keys):
                        gp_test = gp_stuff[dim]
                        model = gp_test[0]
                        likelihood = gp_test[1]
                        ...
                        mean_pred = observed_pred.mean.cpu()
                        mean_pred = mean_pred.numpy()
                        var = observed_pred.variance.cpu()
                        var = var.detach().numpy()

                        # dim_sig = (upper - mean_pred)/2  # 1 std dev, std dev is sqrt(var)
                        dim_sig = var  # variance

                        if dim_sig > sig_2_dict[key]:
                            sig_2_dict[key] = dim_sig
                        if dim_sig < sig_2_dict_min[key]:
                            sig_2_dict_min[key] = dim_sig

                        sig_2 += dim_sig ** 2.0
                        dist_from_mean += (true_y[dim] - mean_pred) ** 2.0

                        bounds.append([lower[0], upper[0]])
                        if true_y[dim] > upper[0] or true_y[dim] < lower[0]:
                            succeeds = False

                    sig_2 = sig_2 ** 0.5
                    dist_from_mean = dist_from_mean ** 0.5

                    if dist_from_mean > max_dist_from_mean:
                        max_dist_from_mean = dist_from_mean
                    ...
                    if total_num <= 30:
                        print(
                            f"input: {np.transpose(np.stack(x_testing))[0]}, f(input): {np.transpose(np.stack(true_y))[0]}")
                        print(f"gp predicted bounds: {bounds} \nSucceeds: {succeeds} \n")
                    if succeeds:
                        num_true += 1

                toc = time.perf_counter()
                print(f"It took {toc - tic:0.4f} seconds to test the data.")
                print(
                    f"{num_true / total_num * 100.} percent pass the check.\nMax distance from mean = {max_dist_from_mean}.")
                print(
                    f"Min distance from mean = {min_dist_from_mean}\nMax variance = {max_2_sig}\nMin variance = {min_2_sig}")
                print(f"Max variance per dim: {sig_2_dict}")
                print(f"Min variance per dim: {sig_2_dict_min}")
                print(f"The above is for mode {mode} and region {region_idx}\n\n")
        else:
            gp_stuff = unknown_dyn_gp[mode]
            num_true = 0
            total_num = 30000
            max_dist_from_mean = 0
            min_dist_from_mean = 9e9
            ...
            dim_first_pass = np.ones([d,1])
            for nums in range(total_num):
                x_testing = [np.random.uniform(test_domain[k][0], test_domain[k][1], 1) for k in keys]
                x_torch = torch.tensor(np.reshape(x_testing, [1, d]), dtype=torch.float32).to(device)
                ...
                if total_num <= 30:
                    print(f"input: {np.transpose(np.stack(x_testing))[0]}, f(input): {np.transpose(np.stack(true_y))[0]}")
                    print(f"gp predicted bounds: {bounds} \nSucceeds: {succeeds} \n")
                if succeeds:
                    num_true += 1
            ...
            toc = time.perf_counter()
            print(f"It took {toc - tic:0.4f} seconds to test the data.")
            print(f"{num_true/total_num*100.} percent pass the check.\nMax distance from mean = {max_dist_from_mean}.")
            print(f"Min distance from mean = {min_dist_from_mean}\nMax 2-sigma = {max_2_sig}\nMin 2-sigma = {min_2_sig}")
            print(f"Max 2 sigma per dim: {sig_2_dict}")
            print(f"Min 2 sigma per dim: {sig_2_dict_min}")
            print(f"The above is for mode {mode}\n\n")
