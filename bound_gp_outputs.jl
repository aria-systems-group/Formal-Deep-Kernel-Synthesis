                ...
                @sync @distributed for idx in specific_extents
                    ...
                    if any([matching_label(label_fn[idx+1], skip_) for skip_ in skip_labels])
                        ...
                        if refinement > 0 && sig_check <= 0*sqrt(dim_sig)
                            ...
                            sig_info_l = sigma_low_bnb(gp, x_L, x_U, theta_vec, theta_vec_2, cK_inv_scaled;
                                                       max_iterations=10, bound_epsilon=1e-2)
                            sig_lower = sqrt(sig_info[3])
                            sig_lower = min(sig_low, sig_lower)
                        end
...
function bound_error_gp(num_regions, num_modes, num_dims, refinement, global_exp_dir, reuse_regions, label_fn, skip_labels)
    nn_bounds_dir = global_exp_dir * "/nn_bounds"
    gp_bounds_dir = global_exp_dir * "/gp_bounds"
        ...
        mode_runtime = @elapsed begin
            for dim in 1:(num_dims::Int)
                ...
                @sync @distributed for idx in specific_extents
                    ...
                    x_L_nn = linear_bounds_nn[idx+1, 1, :]
                    x_U_nn = linear_bounds_nn[idx+1, 2, :]
                    ...
                    if any([matching_label(label_fn[idx+1], skip_) for skip_ in skip_labels])
                        ...
                        mean_bound[idx+1, dim, 1] = mean_info_l[2] + x_L_nn[dim]
                        mean_bound[idx+1, dim, 2] = -mean_info_u[2] + x_U_nn[dim]
