...
def gp_info(n_obs, domain, gp, dim_key, process_noise=None):
    noise_sigma = 0.
    if process_noise is not None:
        noise_sigma = process_noise["sig"]

    if noise_sigma == 0.:
        scale_factor = 1
    else:
        scale_factor = noise_sigma / np.sqrt(1. + 2. / n_obs)

    diam_domain = 0
    for key in list(domain):
        diam_domain += (domain[key][0] - domain[key][1]) ** 2.
    diam_domain = np.sqrt(diam_domain)

    # l2 is kernel length scale squared
    # sig_2 is kernel standard deviation squared
    kernel = gp.kernel_
    kernel_params = kernel.get_params()

    sig_2 = kernel_params["k1__constant_value"]
    l_2 = kernel_params["k2__length_scale"] ** 2.0

    print(f'length scale = {kernel_params["k2__length_scale"]}, noise = {sig_2}')

    sig_inf = np.sqrt(sig_2 * math.exp(-1 / 2 * (diam_domain) ** 2 / l_2))

    lipschitz_bound = 1.0
    RKHS_bound = abs((domain[dim_key][1] - domain[dim_key][0]) / 2.0 + lipschitz_bound * diam_domain) / sig_inf
    B = 1.0 + (1.0 + 2.0 / n_obs) ** (-1.0)
    gamma = 0.5 * n_obs * math.log(B)

    gp_info_dict = {"gamma": gamma, "RKHS": RKHS_bound, "scale": scale_factor}

    return gp_info_dict


def rkhs_bound_by_dim(sigma, epsilon, gp_info_dict, dim_key):
    B = gp_info_dict["RKHS"][dim_key]

    logNoise = 0  # ?
    R = gp_info_dict["scale"] * math.exp(logNoise)

    frac = epsilon / (gp_info_dict["scale"] * sigma)
    bound = 1.
    if frac > B:
        bound = math.exp(-0.5 * (1 / R * (frac - B)) ** 2.0 + gp_info_dict["gamma"] + 1.)

    return bound


def gpy_plot_1D(error_gp, unknown_dyn_list, all_data, unknown_modes_list, mode, domain, global_exp_dir):
    model = error_gp[0]
    likelihood = error_gp[1]

    x1_ = domain["x1"]
    g = unknown_modes_list[mode]  # true dynamics function

    x_train = all_data[mode]["x_train"]
    y_train = all_data[mode]["y_train"]
    x_test = all_data[mode]["x_test"]
    y_test = all_data[mode]["y_test"]
    unknown_dyn_model = unknown_dyn_list[mode]  # estimated dynamics function
    x_torch = torch.tensor(np.transpose(x_test), dtype=torch.float32)
    y_predict = unknown_dyn_model(x_torch).detach().numpy()
    error_obs = np.transpose(y_test) - y_predict

    refinement_dir = global_exp_dir + "/eps_figs"
    if not os.path.isdir(refinement_dir):
        os.mkdir(refinement_dir)

    X = np.arange(x1_[0], x1_[1], 0.01)
    len_x = np.size(X)
    X_ = np.arange(x1_[0], x1_[1], 0.01).reshape((len_x, 1))
    y = g(X_)

    x_torch = torch.tensor(X_, dtype=torch.float32)
    y_predict = unknown_dyn_model(x_torch).detach().numpy()
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        observed_pred = likelihood(model(x_torch))

    lower, upper = observed_pred.confidence_region()
    mean_prediction = observed_pred.mean.numpy()

    y_plus = y_predict.reshape((len_x,)) + upper.detach().numpy()
    y_minus = y_predict.reshape((len_x,)) + lower.detach().numpy()

    plt.plot(X_, y, label="True Dynamics")
    plt.scatter(x_train, y_train, label='Observations')
    plt.plot(X_, y_predict, label="NN Prediction")
    plt.legend()
    plt.xlabel("$x$")
    plt.ylabel("$f(x)$")
    ...
    plt.show(block=False)
    plt.savefig(refinement_dir + '/NN_plot.png')

    plt.clf()

    plt.plot(X_, y, label="True Dynamics")
    plt.scatter(x_test, y_test, label='Observations')
    plt.plot(X_, y_predict + mean_prediction.reshape((len_x, 1)), label="NN + GP Mean Prediction")
    plt.fill_between(X, y_minus, y_plus, alpha=0.5)
    plt.legend()
    plt.xlabel("$x$")
    plt.ylabel("$f(x)$")
    plt.title("NN + GP error regression")

    plt.show(block=False)
    plt.savefig(refinement_dir + '/GP_plot.png')

    plt.clf()

    y_plus = upper.detach().numpy()
    y_minus = lower.detach().numpy()

    plt.plot(X_, y - y_predict, label="True Error")
    plt.scatter(x_test, error_obs, label='Observations')
    plt.plot(X_, mean_prediction, label="Mean Error Prediction")
    plt.fill_between(X, y_minus.reshape((len_x,)), y_plus.reshape((len_x,)), alpha=0.5)
    plt.xticks(np.arange(0, 10, step=0.5))
    plt.grid()
    plt.legend()
    plt.xlabel("$x$")
    plt.ylabel("$f(x)$")
    plt.title("GP error regression")

    plt.show(block=False)
    plt.savefig(refinement_dir + '/GP_error_plot.png')


def deep_kernel_plot(gp_dims, all_data, unknown_modes_list, mode, domain, global_exp_dir):
    gp = gp_dims[0]
    ...
    refinement_dir = global_exp_dir + "/deep_kernel_figs"
    if not os.path.isdir(refinement_dir):
        os.mkdir(refinement_dir)
    ...
    x_torch = torch.tensor(X_, dtype=torch.float32).cuda()

    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        observed_pred = likelihood(model(x_torch))

    lower, upper = observed_pred.confidence_region()
    mean_prediction = observed_pred.mean.cpu()
    ...
    plt.plot(X_, y, label="True Dynamics")
    plt.scatter(x_train, y_train, label='Observations')
    plt.plot(X_, mean_prediction.reshape((len_x, 1)), label="GP Mean Prediction")
    plt.fill_between(X, y_minus.reshape((len_x,)), y_plus.reshape((len_x,)), alpha=0.5)
    plt.legend()
    plt.xlabel("$x$")
    plt.ylabel("$f(x)$")
    ...
    # also plot the kernel correlation at two random points
    np.random.seed(12435435)

    p1 = torch.tensor(np.random.uniform(x1_[0], x1_[1], 1), dtype=torch.float32).cuda()
    p2 = torch.tensor(np.random.uniform(x1_[0], x1_[1], 1), dtype=torch.float32).cuda()
    p3 = torch.tensor(np.random.uniform(x1_[0], x1_[1], 1), dtype=torch.float32).cuda()

    nn_portion = model.feature_extractor
    kernel_fnc = model.covar_module

    with torch.no_grad():
        kernel_inputs = nn_portion.forward(x_torch)
        p1_k = nn_portion.forward(p1)
        p2_k = nn_portion.forward(p2)
        p3_k = nn_portion.forward(p3)

    correlation_vec_1 = kernel_fnc(p1_k, kernel_inputs)
    correlation_vec_1 = correlation_vec_1.evaluate()
    correlation_vec_1 = correlation_vec_1.cpu()
    correlation_vec_1 = correlation_vec_1.detach().numpy()
    correlation_vec_1 = correlation_vec_1.reshape((len_x, 1))

    correlation_vec_2 = kernel_fnc(p2_k, kernel_inputs)
    correlation_vec_2 = correlation_vec_2.evaluate()
    correlation_vec_2 = correlation_vec_2.cpu()
    correlation_vec_2 = correlation_vec_2.detach().numpy()
    correlation_vec_2 = correlation_vec_2.reshape((len_x, 1))

    correlation_vec_3 = kernel_fnc(p3_k, kernel_inputs)
    correlation_vec_3 = correlation_vec_3.evaluate()
    correlation_vec_3 = correlation_vec_3.cpu()
    correlation_vec_3 = correlation_vec_3.detach().numpy()
    correlation_vec_3 = correlation_vec_3.reshape((len_x, 1))

    p1 = p1.cpu()
    p2 = p2.cpu()
    p3 = p3.cpu()

    plt.plot(X_, correlation_vec_1, 'g', label="Correlation 1")
    plt.plot(X_, correlation_vec_2, 'b', label="Correlation 2")
    plt.plot(X_, correlation_vec_3, 'k', label="Correlation 3")
    bottom, top = plt.ylim()
    plt.plot([p1, p1], [bottom, top], 'g--', label="$x_1'$")
    plt.plot([p2, p2], [bottom, top], 'b--', label="$x_2'$")
    plt.plot([p3, p3], [bottom, top], 'k--', label="$x_3'$")
    plt.ylim((bottom, top))
    ...
    plt.show(block=False)
    plt.savefig(refinement_dir + '/kernel_plot.png')

    plt.clf()


def deep_kernel_plot_nD(gp_dims, all_data, dim, mode, domain, global_exp_dir, random_seed=22):
    gp = gp_dims[dim]
    ...
    X_ = np.arange(x1[0], x1[1], 0.01)
    Y_ = np.arange(x2[0], x2[1], 0.01)
    Z_ = np.arange(x3[0], x3[1], 0.1)
    for z in Z_:
        input_vec = [[x, z, y] for x in X_ for y in Y_]
        x_torch = torch.tensor(input_vec, dtype=torch.float32).cuda()

        nn_portion = model.feature_extractor
        kernel_fnc = model.covar_module

        p1 = torch.tensor([[2.98, 1.65, z]], dtype=torch.float32).cuda()
        with torch.no_grad():
            kernel_inputs = nn_portion.forward(x_torch)
            p1_k = nn_portion.forward(p1)
        ...
        corr_surf = np.transpose(correlation_vec_1.reshape([len(X_), len(Y_)]))

        surf = ax.plot_surface(X, Y, corr_surf)
        fig.colorbar(surf)
        plt.savefig(refinement_dir + f'/kernel_plot_{z}.png')
